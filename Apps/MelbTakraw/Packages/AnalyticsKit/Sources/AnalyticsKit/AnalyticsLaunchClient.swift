...
public struct AnalyticsLaunchClient: Sendable {
    ...
    public func checkAccess() async throws -> AnalyticsAvailabilityResponse {
        try await checkAccess(languageCode: Self.currentLanguageCode())
    }

    public func checkAccess(languageCode: String) async throws -> AnalyticsAvailabilityResponse {
        let response: AnalyticsAvailabilityResponse
        switch config.requestStyle {
        case .appIDOnly:
            response = try await send(request: makeAppIDCheckRequest())
        case .launchAnalytics:
            do {
                response = try await send(
                    request: makeLaunchRequest(
                        payload: Self.defaultPayload(config: config, languageCode: languageCode)
                    )
                )
            } catch let error as AnalyticsRequestError where error.shouldRetryWithAppIDPayload {
                response = try await send(request: makeAppIDCheckRequest())
            }
        }
    ...
    private func send(request: URLRequest) async throws -> AnalyticsAvailabilityResponse {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw AnalyticsRequestError(statusCode: statusCode, body: body)
        }
...
private struct AnalyticsRequestError: Error {
    let statusCode: Int
    let body: String

    var shouldRetryWithAppIDPayload: Bool {
        statusCode == 404 || body.localizedCaseInsensitiveContains("Application not found")
    }
}
